    ...
    public function save($post_id)
    {
        ...
        foreach ($this->fields as $field) {
            ...
            if (!in_array($field['type'], array('separator', 'title'))) {
                ...
                $_string = get_option('reveal-modal-string-random');
                if ($name == 'is_reveal_modal' && $value == 'true') {
                    if (strpos($_post->post_name, $_string) == false) {
                        global $wpdb;
                        $_post_name = $_post->post_name . $_string;
                        $wpdb->update($wpdb->posts, array('post_name' => $_post_name), array('id' => $post_id));
                    }
                } elseif ($name == 'is_reveal_modal' && $value == 'false') {
                    global $wpdb;
                    $_post_name = str_replace($_string, '', $_post->post_name);
                    $wpdb->update($wpdb->posts, array('post_name' => $_post_name), array('id' => $post_id));
                }
